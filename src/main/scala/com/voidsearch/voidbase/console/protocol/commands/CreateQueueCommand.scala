...
case class CreateQueueCommand (session: VoidBaseConsoleSession, name: String, size : String) extends VoidBaseConsoleCommand {

  def exec() = {

    session.domain match {
      case "queue" =>  createQueue()
      case "qtree" =>  createQueue()
      case "queuetree" => createQueue()
  ...
  def createQueue() = {
    ...
    client.create(name,Integer.parseInt(size))
  }
